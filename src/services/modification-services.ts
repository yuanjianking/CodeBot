...
import { MODIFICATION_PROMPT_TEMPLATE, ERROR_CODES } from '../constants/index.js';
...
export async function modifyCode(
 codeDirectory: string,
  ...
  try {
    console.log(`开始修改代码目录: ${codeDirectory}`);
    ...
    // 调用Claude进修改
    console.log('正在使用Claude修改代码...');
    await queryClaude(prompt);
    console.log('代码修改完成');

  } catch (error) {
    console.error('代码修改失败:', error);
    throw new Error(`代码修改失败: ${error instanceof Error ? error.message : '未知错误'}`, {
...
function buildAnalysisPrompt(
  ...
  const prompt = MODIFICATION_PROMPT_TEMPLATE
    .replace('{{codeDirectory}}', codeDirectory)
    .replace('{{taskRequirement}}', taskRequirement)
    .replace('{{reportPath}}', `analysis-reports/${codeDirectory}_{最新的时间戳}`);
